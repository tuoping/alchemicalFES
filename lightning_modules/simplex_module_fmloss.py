...
def sample_cond_prob_path_2d(hyperparams, seq, channels, condflow):
    shape = seq.shape
    batchsize = seq.shape[0]
    seq_onehot = torch.nn.functional.one_hot(seq.reshape(-1), num_classes=channels).reshape(*shape, channels)
    t = torch.from_numpy(scipy.stats.expon().rvs(size=batchsize)*hyperparams.time_scale).to(seq.device).float()
    # exponential_dist = torch.distributions.Exponential(torch.tensor(1.0, device=seq.device))
    # t = exponential_dist.sample((batchsize,))*hyperparams.time_scale
    alphas = torch.ones(*shape, channels, device=seq.device)*hyperparams.time0_scale
    alphas = alphas + t[:, None, None, None]*seq_onehot
    xt = torch.distributions.Dirichlet(alphas).sample()
    eye = torch.eye(channels).to(xt)

    B,H,W = shape
    K = channels
    flow_probs = xt
    if not torch.allclose((flow_probs.reshape(B,-1,K)).sum(2), torch.ones((B, H*W), device=seq.device), atol=1e-4) or not (flow_probs >= 0).all():
    ...
    c_factor = condflow.c_factor(xt.cpu().numpy(), t.item())
    ...
    if torch.isnan(c_factor).any():
        ...
        if hyperparams.allow_nan_cfactor:
...
class simplexModule_fmloss(GeneralModule):
    ...
    def general_step(self, batch, batch_idx=None):
        ...
        if self.hyperparams.model == "CNN3D":
            ...
            ut, xt, t = sample_cond_prob_path_2d(self.hyperparams, seq, self.model.alphabet_size, self.condflow)
