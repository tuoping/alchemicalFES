...
class CNNModel2D(nn.Module):
    def __init__(self, args, alphabet_size, num_cls, classifier=False):
        ...
        self.final_conv = nn.Sequential(nn.Conv2d(args.hidden_dim, args.hidden_dim, kernel_size=1),
                                   nn.ReLU(),
                                   nn.Conv2d(args.hidden_dim, self.alphabet_size, kernel_size=1))
        if classifier:
            self.final_conv_classifier = nn.Sequential(nn.Conv2d(args.hidden_dim, args.hidden_dim, kernel_size=1),
                                   nn.ReLU(),
    ...
    def forward(self, seq, t, cls = None, return_embedding=False):
        ...
        B = seq.shape[0]
        if self.classifier:
            final_classifier = self.final_conv_classifier(feat)
            final_classifier = (final_classifier.permute(0,2,3,1))
            feat = self.final_conv(feat)
            if return_embedding:
                embedding = self.cls_head[:1](feat)
                return self.cls_head[1:](embedding), embedding
            else:
                return feat, (self.cls_head(final_classifier).reshape(B,-1,self.num_cls)).sum(dim=1)
        else:
            feat = self.final_conv(feat)
            return feat
