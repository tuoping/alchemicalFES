...
def reversekde(rc_trajs, x_grid, bandwidth=2.):
        ...
        for i, x in enumerate(x_grid):
            ...
            if torch.isinf(density[i]).sum():
                num_Inf+=1
            if torch.isnan(density[i]).sum():
                num_nan+=1
        sum_density = torch.sum(density, axis=1)
        norm_density = density/sum_density[:,None]
        print("HIST:: ", num_Inf, num_nan, torch.sum(sum_density == 0), len(sum_density))
        idxF = torch.where(sum_density != 0.)
        F = -torch.log(norm_density[idxF])
        return F,norm_density[idxF]
...
import glob
def loadmodelprediction(_dirname, epoch, num_batches, header_file = "logits"):
    dirname = _dirname+"/epoch%d_sample%d"%(epoch,num_batches)
    f_logits_t = sorted(glob.glob(os.path.join(dirname, header_file+"_val_step0_inttime*")))
    print(">>> Reading model predictions from: ", dirname)

    logits_t = [np.load(f).astype(np.float16) for f in f_logits_t]
    t = [float(x.replace(os.path.join(dirname, header_file+"_val_step0_inttime"), "").replace(".npy",""))-1 for x in f_logits_t]
    '''
    for ii in range(num_batches):
        ...
        _f_logits_t = sorted(glob.glob(os.path.join(dirname, header_file+"_val_step0_inttime*")))
...
def run_RC_val(T, epoch, header_file = "logits"):
    ### load model predictions
    s_time = time.time()
    logits_t, t = loadmodelprediction(val_dirname[T], epoch, num_batches, header_file=header_file)
    e_time = time.time()
    print("Time series of diffusion = ", t)
    ...
    for ii in range(len(logits_t)):
        ...
        F,H = reversekde(rc,torch.from_numpy(bin_centers),1.)
        assert F.shape == H.shape
        plt.figure()
        plt.hist(rc.numpy(), density=True)
        plt.plot(bin_centers, np.sum(H.numpy(), axis=0))
        plt.savefig(header_file+"-HIST-RC-t%.4f.png"%t_ii, bbox_inches="tight")
        plt.figure()
        plt.plot(bin_centers, -np.logaddexp.reduce(-F.numpy(), axis=0))
        plt.savefig(header_file+"-F-RC-t%.4f.png"%t_ii, bbox_inches="tight")


def run_statistics(T, epoch, header_file="xt"):
    ### load model predictions
    s_time = time.time()
    logits_t, t = loadmodelprediction(val_dirname[T], epoch, num_batches, header_file=header_file)
    ...
    for ii in range(len(seq_t)):
        if ii == 0:
            ...
            t_ii = 10/t[ii]
        ofile_Prob = open(header_file+"-PROB-MAGN-kBT%.2f-t%.4f.dat"%(T,t_ii), "wb")
        ofile_F = open(header_file+"-F-MAGN-kBT%.2f-t%.4f.dat"%(T,t_ii), "wb")
        plt.figure()
        print(">>> PROCESSING:: ")
        ...
        plt.savefig(header_file+"-F-MAGN-kBT%.2f-t%.4f.png"%(T,t_ii), bbox_inches="tight")
        ofile_Prob.flush()
        ofile_F.flush()
        ofile_Prob.close()
        ofile_F.close()
...
run_statistics(T1, epoch1, header_file="logits")
run_statistics(T1, epoch1)
run_RC_val(T1, epoch1)
run_RC_val(T1, epoch1, header_file="xt")
