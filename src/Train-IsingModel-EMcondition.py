...
os.environ["MODEL_DIR"]=f"logs-dir-ising/latt6x6T3.2/kernel3x3_timeembed_symmetrized/clsfreeG/Mloss/guidanceE"
os.environ["work_dir"]=os.environ["MODEL_DIR"]

dataset_dir = ["data/ising-latt6x6-T3.2/buffer.npy"]
...
class Hyperparams():
    def __init__(self, mode=None, hidden_dim=16, num_cnn_stacks=1, lr=5e-4, dropout=0.0, clean_data=False, model="MLP"):
        ...
        if mode is not None and ("RC" in mode or "RC-focal" in mode):
            self.prefactor_RC = 100.
            self.prefactor_CE = 1.00
