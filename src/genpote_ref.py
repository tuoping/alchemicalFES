...
def run_loading():
    ...
    seq_ref_list = {}
    for idx_jj, jj in enumerate([2.0, 2.2, 2.4, 2.8, 3.2, 3.6, 4.0, 4.4, 4.8, 6.0]):
    ...
    ofile_Prob_ref = open("PROB-E-REF.dat", "wb")
    ofile_F_ref = open("F-E-REF.dat", "wb")
    plt.figure()
...
def run_expectation():
    ...
    ofile_expectation_E = open("Expectation-E-REF.dat","wb")
...
def run_appending(Tlist):
    ...
    num_bins = int((max_E-min_E)/2)
    bins = np.linspace(min_E-1./num_bins, max_E+1./num_bins, num_bins+2)
    ofile_Prob_ref = open("PROB-E-REF.dat", "ab")
    ofile_F_ref = open("F-E-REF.dat", "ab")
    ...
    ofile_expectation_E = open("Expectation-E-REF.dat","ab")
    np.savetxt(ofile_expectation_E, Expectation_E_list)
    ofile_expectation_E.close()
